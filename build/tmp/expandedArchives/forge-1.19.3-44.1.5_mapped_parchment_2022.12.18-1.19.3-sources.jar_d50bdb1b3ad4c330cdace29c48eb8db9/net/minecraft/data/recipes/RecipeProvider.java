...
public abstract class RecipeProvider implements DataProvider {
   ...
   protected static void coloredWoolFromWhiteWoolAndDye(Consumer<FinishedRecipe> pFinishedRecipeConsumer, ItemLike pDyedWool, ItemLike pDye) {
      ShapelessRecipeBuilder.shapeless(RecipeCategory.BUILDING_BLOCKS, pDyedWool).requires(pDye).requires(Blocks.WHITE_WOOL).group("wool").unlockedBy("has_white_wool", has(Blocks.WHITE_WOOL)).save(pFinishedRecipeConsumer);
   }
   ...
   protected static void coloredCarpetFromWhiteCarpetAndDye(Consumer<FinishedRecipe> pFinishedRecipeConsumer, ItemLike pDyedCarpet, ItemLike pDye) {
      ShapedRecipeBuilder.shaped(RecipeCategory.DECORATIONS, pDyedCarpet, 8).define('#', Blocks.WHITE_CARPET).define('$', pDye).pattern("###").pattern("#$#").pattern("###").group("carpet").unlockedBy("has_white_carpet", has(Blocks.WHITE_CARPET)).unlockedBy(getHasName(pDye), has(pDye)).save(pFinishedRecipeConsumer, getConversionRecipeName(pDyedCarpet, Blocks.WHITE_CARPET));
   }
   ...
   protected static void bedFromWhiteBedAndDye(Consumer<FinishedRecipe> pFinishedRecipeConsumer, ItemLike pDyedBed, ItemLike pDye) {
      ShapelessRecipeBuilder.shapeless(RecipeCategory.DECORATIONS, pDyedBed).requires(Items.WHITE_BED).requires(pDye).group("dyed_bed").unlockedBy("has_bed", has(Items.WHITE_BED)).save(pFinishedRecipeConsumer, getConversionRecipeName(pDyedBed, Items.WHITE_BED));
   }
