...
public class OverworldBiomes {
   ...
   private static Biome biome(Biome.Precipitation pPrecipitation, float pTemperature, float pDownfall, MobSpawnSettings.Builder pMobSpawnSettings, BiomeGenerationSettings.Builder pGenerationSettings, @Nullable Music pBackgroundMusic) {
      return biome(pPrecipitation, pTemperature, pDownfall, 4159204, 329011, pMobSpawnSettings, pGenerationSettings, pBackgroundMusic);
   }

   private static Biome biome(Biome.Precipitation pPrecipitation, float pTemperature, float pDownfall, int pWaterColor, int pWaterFogColor, MobSpawnSettings.Builder pMobSpawnSettings, BiomeGenerationSettings.Builder pGenerationSettings, @Nullable Music pBackgroundMusic) {
      return (new Biome.BiomeBuilder()).precipitation(pPrecipitation).temperature(pTemperature).downfall(pDownfall).specialEffects((new BiomeSpecialEffects.Builder()).waterColor(pWaterColor).waterFogColor(pWaterFogColor).fogColor(12638463).skyColor(calculateSkyColor(pTemperature)).ambientMoodSound(AmbientMoodSettings.LEGACY_CAVE_SETTINGS).backgroundMusic(pBackgroundMusic).build()).mobSpawnSettings(pMobSpawnSettings.build()).generationSettings(pGenerationSettings.build()).build();
   }
   ...
   public static Biome oldGrowthTaiga(HolderGetter<PlacedFeature> pPlacedFeatureGetter, HolderGetter<ConfiguredWorldCarver<?>> pConfiguredWorldCarverGetter, boolean pIsSpruce) {
      ...
      return biome(Biome.Precipitation.RAIN, pIsSpruce ? 0.25F : 0.3F, 0.8F, mobspawnsettings$builder, biomegenerationsettings$builder, music);
   }
   ...
   private static Biome baseOcean(MobSpawnSettings.Builder pMobSpawnSettings, int pWaterColor, int pWaterFogColor, BiomeGenerationSettings.Builder pGenerationSettings) {
      return biome(Biome.Precipitation.RAIN, 0.5F, 0.5F, pWaterColor, pWaterFogColor, pMobSpawnSettings, pGenerationSettings, NORMAL_MUSIC);
   }
