...
public class Sniffing<E extends Warden> extends Behavior<E> {
   ...
   protected void stop(ServerLevel pLevel, E pEntity, long pGameTime) {
      ...
      pEntity.getBrain().getMemory(MemoryModuleType.NEAREST_ATTACKABLE).filter(pEntity::canTargetEntity).ifPresent((p_253535_) -> {
         if (pEntity.closerThan(p_253535_, 6.0D, 20.0D)) {
            pEntity.increaseAngerAt(p_253535_);
         }

         if (!pEntity.getBrain().hasMemoryValue(MemoryModuleType.DISTURBANCE_LOCATION)) {
            WardenAi.setDisturbanceLocation(pEntity, p_253535_.blockPosition());
         }
