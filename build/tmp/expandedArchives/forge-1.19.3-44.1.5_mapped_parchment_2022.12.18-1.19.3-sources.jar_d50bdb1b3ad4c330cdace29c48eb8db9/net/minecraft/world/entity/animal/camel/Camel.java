...
public class Camel extends AbstractHorse implements PlayerRideableJumping, RiderShieldingMount, Saddleable {
   ...
   public void addAdditionalSaveData(CompoundTag pCompound) {
      ...
      pCompound.putBoolean("IsSitting", this.getPose() == Pose.SITTING);
   ...
   public void readAdditionalSaveData(CompoundTag pCompound) {
      ...
      if (pCompound.getBoolean("IsSitting")) {
         this.setPose(Pose.SITTING);
      }

      this.resetLastPoseChangeTick(pCompound.getLong("LastPoseTick"));
   }
   ...
   public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData, @Nullable CompoundTag pDataTag) {
      ...
      this.entityData.set(LAST_POSE_CHANGE_TICK, pLevel.getLevel().getGameTime() - 52L);
   ...
   protected float getDrivenMovementSpeed(LivingEntity pEntity) {
      float f = pEntity.isSprinting() && this.getJumpCooldown() == 0 ? 0.1F : 0.0F;
      return (float)this.getAttributeValue(Attributes.MOVEMENT_SPEED) + f;
   }

   protected boolean mountIgnoresControllerInput(LivingEntity pEntity) {
      boolean flag = this.isInPoseTransition();
      if (this.isPoseSitting() && !flag && pEntity.zza > 0.0F) {
         this.standUp();
      }

      return this.refuseToMove() || super.mountIgnoresControllerInput(pEntity);
   }

   public boolean canJump(Player pPlayer) {
      return !this.refuseToMove() && this.getControllingPassenger() == pPlayer && super.canJump(pPlayer);
   }
   ...
   protected void onLeashDistance(float pDistance) {
      if (pDistance > 6.0F && this.isPoseSitting() && !this.isInPoseTransition()) {
         this.standUp();
      }
