...
public class InteractWithDoor {
   ...
   private static boolean areOtherMobsComingThroughDoor(LivingEntity pEntity, BlockPos pPos, Optional<List<LivingEntity>> pNearestLivingEntities) {
      return pNearestLivingEntities.isEmpty() ? false : pNearestLivingEntities.get().stream().filter((p_248306_) -> {
         return p_248306_.getType() == pEntity.getType();
      }).filter((p_253528_) -> {
         return pPos.closerToCenterThan(p_253528_.position(), 2.0D);
      }).anyMatch((p_258454_) -> {
