...
public class PiglinAi {
   ...
   protected static void wasHurtBy(Piglin pPiglin, LivingEntity pTarget) {
      if (!(pTarget instanceof Piglin)) {
         ...
         getAvoidTarget(pPiglin).ifPresent((p_248390_) -> {
            if (p_248390_.getType() != pTarget.getType()) {
   ...
   protected static void broadcastAngerTarget(AbstractPiglin pPiglin, LivingEntity pTarget) {
      getAdultPiglins(pPiglin).forEach((p_248393_) -> {
         if (pTarget.getType() != EntityType.HOGLIN || p_248393_.canHunt() && ((Hoglin)pTarget).canBeHunted()) {
            setAngerTargetIfCloserThanCurrent(p_248393_, pTarget);
         }
      });
   }
