...
public class LootTable {
   ...
   /**
    * Create a wrapped {@link Consumer} which will split stacks according to their maximum stack size before passing
    * them on to the given {@code stackConsumer}.
    */
   public static Consumer<ItemStack> createStackSplitter(LootContext pContext, Consumer<ItemStack> pStackConsumer) {
      return (p_248509_) -> {
         if (p_248509_.isItemEnabled(pContext.getLevel().enabledFeatures())) {
            if (p_248509_.getCount() < p_248509_.getMaxStackSize()) {
               pStackConsumer.accept(p_248509_);
            } else {
   ...
   public void getRandomItems(LootContext pContextData, Consumer<ItemStack> pStacksOut) {
      this.getRandomItemsRaw(pContextData, createStackSplitter(pContextData, pStacksOut));
   }
