...
public class PrimaryLevelData implements ServerLevelData, WorldData {
   ...
   public PrimaryLevelData(LevelSettings pSettings, WorldOptions pWorldOptions, PrimaryLevelData.SpecialWorldProperty pSpecialWorldProperty, Lifecycle pWorldGenSettingsLifecycle) {
      this((DataFixer)null, SharedConstants.getCurrentVersion().getWorldVersion(), (CompoundTag)null, false, 0, 0, 0, 0.0F, 0L, 0L, 19133, 0, 0, false, 0, false, false, false, WorldBorder.DEFAULT_SETTINGS, 0, 0, (UUID)null, Sets.newLinkedHashSet(), new TimerQueue<>(TimerCallbacks.SERVER_CALLBACKS), (CompoundTag)null, new CompoundTag(), pSettings.copy(), pWorldOptions, pSpecialWorldProperty, pWorldGenSettingsLifecycle);
   }

   public static PrimaryLevelData parse(Dynamic<Tag> pDynamic, DataFixer pFixerUpper, int pPlayerDataVersion, @Nullable CompoundTag pLoadedPlayerTag, LevelSettings pSettings, LevelVersion pLevelVersion, PrimaryLevelData.SpecialWorldProperty pSpecialWorldProperty, WorldOptions pWorldOptions, Lifecycle pWorldGenSettingsLifecycle) {
      ...
      }).collect(Collectors.toCollection(Sets::newLinkedHashSet)), new TimerQueue<>(TimerCallbacks.SERVER_CALLBACKS, pDynamic.get("ScheduledEvents").asStream()), (CompoundTag)pDynamic.get("CustomBossEvents").orElseEmptyMap().getValue(), compoundtag, pSettings, pWorldOptions, pSpecialWorldProperty, pWorldGenSettingsLifecycle).withConfirmedWarning(pWorldGenSettingsLifecycle != Lifecycle.stable() && pDynamic.get("confirmedExperimentalSettings").asBoolean(false));
   }
