...
public class CreateWorldScreen extends Screen {
   ...
   public static void openFresh(Minecraft pMinecraft, @Nullable Screen pLastScreen) {
      ...
      CreateWorldScreen createWorldScreen = new CreateWorldScreen(pLastScreen, new WorldDataConfiguration(new DataPackConfig(ImmutableList.of("vanilla"), ImmutableList.of()), FeatureFlags.DEFAULT_FLAGS), new WorldGenSettingsComponent(completablefuture.join(), Optional.of(WorldPresets.NORMAL), OptionalLong.empty()));
      pMinecraft.setScreen(createWorldScreen);
      createWorldScreen.tryApplyNewDataPacks(packrepository);
   }

   public static CreateWorldScreen createFromExisting(@Nullable Screen pLastScreen, LevelSettings pLevelSettings, WorldCreationContext pCreationContext, @Nullable Path pTempDataPackDir) {
      CreateWorldScreen createworldscreen = new CreateWorldScreen(pLastScreen, pCreationContext.dataConfiguration(), new WorldGenSettingsComponent(pCreationContext, WorldPresets.fromSettings(pCreationContext.selectedDimensions().dimensions()), OptionalLong.of(pCreationContext.options().seed())));
      createworldscreen.initName = pLevelSettings.levelName();
      createworldscreen.commands = pLevelSettings.allowCommands();
      createworldscreen.commandsChanged = true;
      createworldscreen.difficulty = pLevelSettings.difficulty();
      createworldscreen.gameRules.assignFrom(pLevelSettings.gameRules(), (MinecraftServer)null);
   ...
   private CreateWorldScreen(@Nullable Screen pLastScreen, WorldDataConfiguration pDataConfiguration, WorldGenSettingsComponent pWorldGenSettingsComponent) {
