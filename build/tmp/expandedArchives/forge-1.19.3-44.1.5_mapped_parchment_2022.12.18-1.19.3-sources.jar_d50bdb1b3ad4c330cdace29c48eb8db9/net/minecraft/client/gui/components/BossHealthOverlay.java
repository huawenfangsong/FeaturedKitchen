...
public class BossHealthOverlay extends GuiComponent {
   ...
   private void drawBar(PoseStack pPoseStack, int pX, int pY, BossEvent pBossEvent, int p_232474_, int p_232475_) {
      this.blit(pPoseStack, pX, pY, 0, pBossEvent.getColor().ordinal() * 5 * 2 + p_232475_, p_232474_, 5);
      if (pBossEvent.getOverlay() != BossEvent.BossBarOverlay.PROGRESS) {
         RenderSystem.enableBlend();
         RenderSystem.defaultBlendFunc();
         this.blit(pPoseStack, pX, pY, 0, 80 + (pBossEvent.getOverlay().ordinal() - 1) * 5 * 2 + p_232475_, p_232474_, 5);
