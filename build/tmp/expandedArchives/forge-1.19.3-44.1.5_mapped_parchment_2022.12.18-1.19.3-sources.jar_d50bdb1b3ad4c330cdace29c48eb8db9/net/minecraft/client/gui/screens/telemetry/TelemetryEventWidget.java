...
public class TelemetryEventWidget extends AbstractScrollWidget {
   ...
   protected void renderContents(PoseStack pPoseStack, int pMouseX, int pMouseY, float pPartialTick) {
      ...
      pPoseStack.pushPose();
      pPoseStack.translate((double)j, (double)i, 0.0D);
      this.content.container().render(pPoseStack, pMouseX, pMouseY, pPartialTick);
      pPoseStack.popPose();
   }
   ...
   static class ContentBuilder {
      ...
      public ContentBuilder(int pWidth) {
         ...
         this.helper.addChild(SpacerWidget.width(pWidth));
      ...
      public void addLine(Font pFont, Component pMessage, int pPadding) {
         this.helper.addChild(MultiLineTextWidget.create(this.width, pFont, pMessage), this.helper.newCellSettings().paddingBottom(pPadding));
         this.narration.append(pMessage).append("\n");
      }

      public void addHeader(Font pFont, Component pMessage) {
         this.helper.addChild(MultiLineTextWidget.createCentered(this.width - 64, pFont, pMessage), this.alignHeader);
         this.narration.append(pMessage).append("\n");
      }

      public void addSpacer(int pHeight) {
         this.helper.addChild(SpacerWidget.height(pHeight));
      }
