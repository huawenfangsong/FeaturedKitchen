...
public class LegacyUnicodeBitmapsProvider implements GlyphProvider {
   ...
   public LegacyUnicodeBitmapsProvider(ResourceManager pResourceManager, byte[] pSizes, String pTexturePattern) {
      ...
      for(int i = 0; i < 256; ++i) {
         int j = i * 256;
         set.add(getSheetLocation(pTexturePattern, j));
      }

      String s = getCommonSearchPrefix(set);
      Map<ResourceLocation, CompletableFuture<NativeImage>> map = new HashMap<>();
      pResourceManager.listResources(s, set::contains).forEach((p_248801_, p_248818_) -> {
      ...
      for(int k = 0; k < 256; ++k) {
         int l = k * 256;
         int i1 = k;
         ResourceLocation resourcelocation = getSheetLocation(pTexturePattern, l);
            ...
            list.add(completablefuture.thenAcceptAsync((p_249925_) -> {
               if (p_249925_ != null) {
                  if (p_249925_.getWidth() == 256 && p_249925_.getHeight() == 256) {
                     ...
                     this.sheets[i1] = new LegacyUnicodeBitmapsProvider.Sheet(pSizes, p_249925_);
                  } else {
                     p_249925_.close();
                     Arrays.fill(pSizes, l, l + 256, (byte)0);
                  }
   ...
   public GlyphInfo getGlyph(int pCharacter) {
      if (pCharacter >= 0 && pCharacter < this.sizes.length) {
         int i = pCharacter / 256;
         LegacyUnicodeBitmapsProvider.Sheet legacyunicodebitmapsprovider$sheet = this.sheets[i];
         return legacyunicodebitmapsprovider$sheet != null ? legacyunicodebitmapsprovider$sheet.getGlyph(pCharacter) : null;
      } else {
         return null;
      }
   }
