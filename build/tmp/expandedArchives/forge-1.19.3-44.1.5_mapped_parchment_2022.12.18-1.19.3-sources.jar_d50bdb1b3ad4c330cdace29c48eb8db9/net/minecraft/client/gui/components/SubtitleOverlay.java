   ...
   public void render(PoseStack pPoseStack) {
      ...
      if (this.isListening && !this.subtitles.isEmpty()) {
         ...
         for(SubtitleOverlay.Subtitle subtitleoverlay$subtitle1 : this.subtitles) {
            ...
            pPoseStack.pushPose();
            pPoseStack.translate((float)this.minecraft.getWindow().getGuiScaledWidth() - (float)l * 1.0F - 2.0F, (float)(this.minecraft.getWindow().getGuiScaledHeight() - 35) - (float)(i * (i1 + 1)) * 1.0F, 0.0F);
            ...
            if (!flag) {
               if (d0 > 0.0D) {
                  drawString(pPoseStack, this.minecraft.font, ">", l - this.minecraft.font.width(">"), -j1, j2);
               } else if (d0 < 0.0D) {
                  drawString(pPoseStack, this.minecraft.font, "<", -l, -j1, j2);
               }
            }

            drawString(pPoseStack, this.minecraft.font, component, -k1 / 2, -j1, j2);
            pPoseStack.popPose();
            ++i;
         }
